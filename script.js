...
function makeNewGrid() {
  ...
  buttonDiv.insertAdjacentElement("beforebegin", gridContainer);
  if (passedOnce === true) {
    numberOfSquaresPerSide = prompt(
      "Enter number of squares per side of grid."
    );
  }
  passedOnce = true;
  ...
  while (
    numberOfSquaresPerSide > 100 ||
    Object.is(numberOfSquaresPerSide, NaN) ||
    typeof numberOfSquaresPerSide !== "number" ||
    numberOfSquaresPerSide < 1
  ) {
    numberOfSquaresPerSide = prompt(
      "Try again, please enter a number less than 100 and more than 0"
    );
    if (numberOfSquaresPerSide === "" || numberOfSquaresPerSide === null) {
      numberOfSquaresPerSide = 16;
    }
  ...
  for (let i = 0; i < numberOfSquaresPerSide; i++) {
    ...
    for (let i = 0; i < numberOfSquaresPerSide; i++) {
      ...
      div.setAttribute(
        "style",
        `width: ${480 / numberOfSquaresPerSide}px; height: ${
          480 / numberOfSquaresPerSide
        }px;`
      );
      ...
      div.addEventListener("mouseenter", () => {
        if (toolChosen === "draw") {
          ...
          div.style.opacity =
            Number(div.style.opacity) + Number(opacitySetting.value);
